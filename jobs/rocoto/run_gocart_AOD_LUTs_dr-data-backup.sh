...
for ifield in ${allfields}; do
    ...
    if [ ${ifield} = "emembckg" -o ${ifield} = "ememanal" ]; then
	   ...
	   memstr=`printf %03d $imem`
           ...
           #export gdatadir=${ROTDIR}/${enkfopt}gdas.${gyy}${gmm}${gdd}/${ghh}/${memopt}${memstr}/
           export hofxdir=${ROTDIR}/${enkfopt}gdas.${cyy}${cmm}${cdd}/${chh}/${memopt}${memstr}
	   mkdir -p  ${hofxdir}
       ...
       #export gdatadir=${ROTDIR}/${enkfopt}gdas.${gyy}${gmm}${gdd}/${ghh}/${memopt}/
       export hofxdir=${ROTDIR}/${enkfopt}gdas.${cyy}${cmm}${cdd}/${chh}/${memopt}/
       mkdir -p ${hofxdir}
       echo "Running run_hofx_nomodel_AOD_LUTs for "${ifield}-${trcr_suffix}
       echo ${gdatadir}
       echo ${hofxdir}
...
### Create the coupler files
if [ ! -s ${gdatadir}/${cyy}${cmm}${cdd}.${chh}0000.coupler.res.ges ]; then
cat << EOF > ${gdatadir}/${cyy}${cmm}${cdd}.${chh}0000.coupler.res.ges
     2        (Calendar: no_calendar=0, thirty_day_months=1, julian=2, gregorian=3, noleap=4)
  ${gyy}    ${gmm}    ${gdd}    ${ghh}     0     0        Model start time:   year, month, day, hour, minute, second
  ${cyy}    ${cmm}    ${cdd}    ${chh}     0     0        Current model time: year, month, day, hour, minute, second
EOF
fi
