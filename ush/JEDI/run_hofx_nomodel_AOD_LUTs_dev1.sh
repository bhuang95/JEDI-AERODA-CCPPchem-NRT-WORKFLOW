...
${nln} ${JEDIDir}/geos-aero/test/testinput/geosaod_aeronet.rc ./geosaod_aeronet.rc
fi
...
if [ $AODTYPE = "VIIRS" ]; then
    ...
    obsfile=${ObsDir}/${obsstr}/AERONET_AOD.${obsstr}.nc
    sensorid=aeronet
    obsin=aod_aeronet_obs_${obsstr}.nc4
    obsout=aod_aeronet_hofx_3dvar_LUTs_${obsstr}.nc4
    obsoutproc=aod_aeronet_hofx_3dvar_LUTs_${obsstr}
    ${nln} ${obsfile} ${obsin}
else
    echo "AODTYBE must be VIIRS or MODIS or AERONET; exit this program now!"
    exit 1
fi
