...
if [ $sat = "j01" ]; then
    ...
    FINALFILEv1_npp="${AODTYPE}_AOD_npp.${CDATE}.iodav1.nc"
    FINALFILEv2_npp="${AODTYPE}_AOD_npp.${CDATE}.nc"
    ...
    ncatted -O -a satellite,global,m,c,"NPP" ${FINALFILEv1} ${FINALFILEv1_npp}
    echo "IODA_UPGRADE for ${FINALFILEv1}"
    ${IODAUPGRADEREXEC} ${FINALFILEv1} ${FINALFILEv2}
    ${IODAUPGRADEREXEC} ${FINALFILEv1_npp} ${FINALFILEv2_npp}
    err=$?
    if [[ $err -eq 0 ]]; then
        /bin/mv ${FINALFILEv1}  ${AODOUTDIR}/
        /bin/mv ${FINALFILEv2}  ${AODOUTDIR}/
        /bin/mv ${FINALFILEv1_npp}  ${AODOUTDIR}/
        /bin/mv ${FINALFILEv2_npp}  ${AODOUTDIR}/
        err=$?
    else
