...
export NMEM_EFCSGRP_VTSM=$((NMEM_EFCSGRP * 2))
export ENSEND=$((NMEM_EFCSGRP_VTSM * ENSGRP + NMEM_AERO))
export ENSBEG=$((ENSEND - NMEM_EFCSGRP_VTSM + 1))
...
export DATA=${DATA:-${DATAROOT}/hofx_aod_vtsm.$$}
...
if [ ${ENSGRP} -eq 1 ]; then
    testfile=${ROTDIR}/enkfgdas.${gyy}${gmm}${gdd}/${ghh}/ensmean_VTSM/RESTART/${cprefix}.fv_tracer.res.tile1.nc.ges
...
testfile=${ROTDIR}/enkfgdas.${gyy}${gmm}${gdd}/${ghh}/mem021/RESTART/${cprefix}.fv_tracer.res.tile1.nc.ges
