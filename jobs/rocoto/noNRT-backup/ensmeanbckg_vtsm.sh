...
# Caclculate mean of VTSM-polulated ensemble 
if [[ $VTSM == 'TRUE' ]]; then
    NMEM_AERO=$(expr ${NMEM_AERO} \* 3)
cat > ensmean.nml <<EOF
&ensmean_nml
varnames =  'sphum','bc1','bc2','oc1','oc2','sulf','dust1','dust2','dust3','dust4','dust5','seas1','seas2','seas3','seas4','seas5'
/
...
while [[ $itile -le 6 ]]; do
  ...
  if [[ ! -r ${analdir}/ensmean_VTSM/RESTART/ ]]; then
    mkdir -p ${analdir}/ensmean_VTSM/RESTART/
  fi
  $NMV restart.ensmean ${analdir}/ensmean_VTSM/RESTART/${yyyymmdd}.${hh}0000.fv_tracer.res.tile${itile}.nc.ges
  if [[ $itile == 1 ]]; then
    $NCP $ROTDIR/enkfgdas.${PDY}/${cyc}/mem001/RESTART/${yyyymmdd}.${hh}0000.coupler.res.ges ${analdir}/ensmean_VTSM/RESTART/.
    $NCP $ROTDIR/enkfgdas.${PDY}/${cyc}/mem001/RESTART/${yyyymmdd}.${hh}0000.fv_core.res.nc.ges ${analdir}/ensmean_VTSM/RESTART/.
  fi
  ((itile=itile+1))
done
...
while [[ $itile -le 6 ]]; do
  ...
  $NMV restart.ensmean ${analdir}/ensmean_VTSM/RESTART/${yyyymmdd}.${hh}0000.fv_core.res.tile${itile}.nc.ges
  ((itile=itile+1))
  while [[ $nanal -le $NMEM_AERO ]]; do
    charnanal=mem`printf %03i $nanal`
    unlink restart.${charnanal}
